...
**Idioma:** [English](../../README.md) · [Deutsch](deDE.md) · Español (España) · [Español (México)](esMX.md) · [Français](frFR.md) · [Italiano](itIT.md) · [한국어](koKR.md) · [Português (Brasil)](ptBR.md) · [Русский](ruRU.md) · [简体中文](zhCN.md) · [繁體中文](zhTW.md)
...
- **Web:** [https://wow2.xyz/](https://wow2.xyz/)
...
Esto es un espejo. No abras incidencias ni pull requests aquí. Usa el repo de la Suite o la página de soporte.

Los pasos **normales** (GitHub Desktop) y los **avanzados** (línea de comandos y enlaces para no volver a copiar carpetas) están en esta misma página.
...
- Una carpeta en el ordenador para guardar este repo (Documentos vale)
...
Si usas GitHub Desktop, **no** hace falta instalar Git aparte. Desktop ya lo incluye.

Dirección del repositorio (la pegarás más adelante):
...
Si esas rutas no existen, el juego está instalado en otro sitio:

1. Abre la aplicación **Battle.net**.
2. Selecciona **World of Warcraft**.
3. Abre el **engranaje** / opciones junto a Jugar.
4. En **Ajustes del juego** busca la carpeta de instalación, o usa **Mostrar carpeta** / **Abrir carpeta**.
...
2. Descarga el instalador para **Windows** o **macOS**. La web elige el correcto.
...
5. En **Local path** elige una carpeta fácil de recordar, por ejemplo:

   - Windows: `C:\Users\TUNOMBRE\Documents\OneWoW_Suite_PlayerRepo`
   - Mac: `/Users/TUNOMBRE/Documents/OneWoW_Suite_PlayerRepo`

   Esta carpeta **no** es AddOns de WoW. Es una copia de trabajo. En el siguiente paso copias o enlazas desde aquí hacia AddOns.

6. Pulsa **Clone** y espera a que aparezcan los archivos.
...
**Si usas Catalog**, copia también todas las carpetas `OneWoW_CatalogData_*` o esas pestañas saldrán vacías.
...
No hace falta copiarlo todo. Solo lo que vayas a activar.
...
Tras copiar, `AddOns` debe verse así:
...
1. En la **selección de personaje**, pulsa **Addons**.
...
1. Abre **GitHub Desktop**.
2. Comprueba que este repositorio está seleccionado arriba.
3. Pulsa **Fetch origin**. Consulta GitHub por archivos nuevos.
4. Si aparece **Pull origin**, púlalo. Los cambios llegan a tu carpeta clonada.

Si **copiaste** las carpetas a AddOns, vuelve a copiarlas después de cada pull y sustituye las antiguas. Windows y macOS pedirán reemplazar: acepta.

Si usaste **uniones o enlaces simbólicos** (sección avanzada), el Pull basta. El juego ya apunta a esta carpeta.
...
Un ZIP es una foto completa cada vez. No se actualiza solo. Mejor Desktop o Git si quieres hacer `pull`. **CurseForge** y el ZIP de Discord también valen si no quieres Git.

---

## Avanzado — Git en la terminal

Si te manejas en una terminal. Instala [Git](https://git-scm.com/downloads) primero ([Windows](https://git-scm.com/download/win) · [macOS](https://git-scm.com/download/mac)). En Mac también puedes usar `xcode-select --install`.
...
Luego copia (o enlaza) las carpetas `OneWoW*` en `Interface\AddOns` como en la instalación normal.
...
Si copiaste carpetas, vuelve a copiarlas a AddOns. Si las enlazaste, `git pull` es toda la actualización.

---

## Avanzado — uniones y enlaces simbólicos (sin volver a copiar)

Un **enlace** hace que `AddOns\OneWoW` apunte a `...\OneWoW_Suite_PlayerRepo\OneWoW`. Después, Pull en Desktop o `git pull` actualiza los archivos que el juego ya usa.

- Cierra World of Warcraft primero.
- Clona el repo con Desktop o Git **antes** de crear enlaces (las carpetas de destino deben existir).
- Si ya **copiaste** `OneWoW` a AddOns, borra o renombra esa copia. No se puede crear el enlace si ya hay una carpeta real con el mismo nombre.
- Un enlace por cada carpeta de addon que quieras. Empieza por `OneWoW`.
- Borrar la unión o el enlace en AddOns quita el **enlace**, no tu clon.
...
Si Windows dice que la ruta ya existe, aún hay una carpeta en AddOns. Quítala o renuómbrala y vuelve a ejecutar el comando.
...
Borra solo la carpeta enlazada **dentro de AddOns**. El clon `OneWoW_Suite_PlayerRepo` se queda. Puedes volver a copiar carpetas si quieres.
...
| **Datos de Catalog** | `OneWoW_CatalogData_Journal`, `OneWoW_CatalogData_Vendors`, `OneWoW_CatalogData_Tradeskills`, `OneWoW_CatalogData_Quests` |
...
| El Pull va bien pero el juego se ve antiguo | Copiaste una vez. Vuelve a copiar tras el Pull o usa uniones/enlaces. |
| Fallan `mklink` / `ln` | Ya hay una carpeta real con ese nombre en AddOns, o la ruta está mal. |
| Los addons no cargan | Confirma `_retail_` y que las carpetas están directamente en `AddOns`. |
