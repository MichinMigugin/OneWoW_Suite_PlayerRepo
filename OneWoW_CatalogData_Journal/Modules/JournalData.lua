...
--- Merge older outdoor zone cards from Extended Data. Idempotent per map.
---@param byExp table
function JournalData:RegisterZoneMembership(byExp)
    if not byExp then
        return
    end
    ns.JournalZoneMembership = ns.JournalZoneMembership or {}
    extrasCountByKey = nil
    extrasRareCountByKey = nil
    if self.extrasReady then
        wipe(self.extrasReady)
    end
    if self.extrasByKey then
        wipe(self.extrasByKey)
    end
    for exp, cards in pairs(byExp) do
        ns.JournalZoneMembership[exp] = ns.JournalZoneMembership[exp] or {}
        for mapID, info in pairs(cards) do
            info.source = "extended"
            ns.JournalZoneMembership[exp][mapID] = info
            if self.journalCache then
                local key = self.CacheKey(exp, mapID, "zone")
                if not self.journalCache[key] then
                    self.journalCache[key] = MakeCacheEntry(
                        exp,
                        mapID,
    ...
    ns:FireScanCallbacks("zone_membership")
end
