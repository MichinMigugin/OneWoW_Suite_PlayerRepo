...
**Maintenance:** curated group IDs live in OneWoW_Devs `bin/season_bonus_list_ids.py`. After a new Midnight season’s `ItemBonusListGroup` block appears in OneWoW_Devs `.wow_db2`, add those groups to the mapping and run `python bin/season_bonus_list_ids.py generate` from OneWoW_Devs. There is no DisplaySeason FK.
